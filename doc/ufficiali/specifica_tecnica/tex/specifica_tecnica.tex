...
\lfoot{Specifica tecnica v0.6.3}
\cfoot{}
...
\begin{tabbing}
	...
	\parbox{0.3\textwidth}{\textbf{Verifica: }} \> \parbox{0.5\textwidth}{Beggiato Andrea} \\
...
\thispagestyle{plain}
\title{\begin{Huge}\textsc{Progetto SIGEOL}\end{Huge} \\ \textit{Specifica Tecnica \\ v0.6.3}}
\author{Redazione: Freo Matteo, Barbiero Mattia, Scortegagna Carlo}
\maketitle
\medskip
\begin{center}
...
Questa componente si relaziona con il Controller ed il Middle Man fornendo i dati che vengono richiesti e salvando le eventuali modifiche al \underline{database}.
...
La componente Middle Man è estranea al \underline{pattern} MVC e si occupa di gestire quelle richieste che a causa del tempo di calcolo, renderebbero inutilizzabile ogni altra funzione di consultazione dell'applicazione web. Questa componente sarà implementata tramite un \underline{plugin} per \underline{Ruby on Rails} denominato BackgroundRB.
\subsubsection{Relazione d'uso con le altre componenti}
...
La componente Algorithm non inclusa nel \underline{pattern} MVC si occupa di calcolare effettivamente l'orario delle lezioni, rispettando i vincoli inseriti nel sistema. Dagli studi di ricerca operativa sono noti diversi algoritmi per la ricerca di una soluzione ottimale di un problema sottoposto a vincoli. E' in fase di valutazione, da parte del team QuiXoft, la scelta del particolare tipo di algoritmo.
...
Non è quindi necessario illustrare nella specifica tecnica la struttura del database, in quanto verrà implementato automanticamente da Rails a partire dalla struttura delle classi. 
