...
Il componente \verb|MiddleMan| effettua le seguenti operazioni:
\begin{itemize}
\item riceve le richieste, da parte dell'applicazione, di aggiunta di nuove date per la generazione dell'orario
\item assegna alla componente \verb|Algorithm| la generazione dell'orario
\item segnala all'applicazione la fine del calcolo 
\end{itemize}
\includegraphics[scale=0.34]{images/MiddleMan_class_diagram.png}
\subsubsection{SchedulerServlet}
Servlet che si occupa di ricevere le richiesteb (POST e GET) dall'applicazione:
\begin{description}
\item[POST] 
Parametri: course, date
crea una nuovo trigger che si attiverà alla data (date) per il corso (course) 
\item[GET] 
Parametri: course, inputfile,timeout
richiama la componente \verb|Algorithm| passandogli i parametri ricevuti
\end{description}
\subsubsection{SchedulerJobListener}
Classe richiamata all'esecuzione del trigger.
Si occupa di segnalare all'applicazione l'avvio del calcolo dell'orario per quel determinato corso.


\subsection{Componente MiddleMan}
\includegraphics[scale=0.34]{images/algoritm_class_diagram.png}
