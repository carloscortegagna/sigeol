...
\hline
\hline
\textit{8 giugno 2009} & 2.0.0 & Approvazione del responsabile e passaggio allo stato di Formale  \\
\hline
\textit{29 maggio 2009} & 1.9.0 & Modifiche della disposizione dei paragrafi ed aggiunta di testo  \\
\hline
\textit{20 maggio 2009} & 1.6.0 & Aggiornamento dei digrammi delle classi  \\
\hline
\textit{17 maggio 2009} & 1.4.0 & Correzione dei digrammi delle classi  \\
\hline
\textit{8 maggio 2009} & 1.2.0 & Correzioni ortografiche varie \\
\hline
...
\item[\textbf{ActiveRecord}]
Secondo il \underline{pattern} ActiveRecord esiste una relazione molto stretta fra tabella e classe, colonne e attributi della classe.
\begin{itemize}
...
Queste classi fanno parte della componente model ed ereditatano ed estendono una classe astratta (\verb|ActiveRecord:Base|).
...
Invece per creare una validazione è sufficiente ridefinire i metodi \verb|validate|,\\ \verb|validate_on_update|,\verb|validate_on_create| ereditati dalla classe \verb|ActiveRecord:Base|.
\begin{itemize}
...
ActiveRecord implementa e mette a disposizione: one-to-one, one-to-many, many-to-many.
...
La sezione \verb|PREFERENCES| identifica la sezione nella quale vengono elencate tutte le preferenze degli insegnamenti: rappresenta il desiderio di non tenere una lezione di un determinato insegnamento. Ogni riga di questa sezione rappresenta una singola preferenza, e si attiene alla struttura seguente:
\begin{verbatim}
 CourseID | #Day | #TimeSlot
\end{verbatim}
