...
Uno user può essere o un insegnante o una segreteria didattica.
...
Per implementare questa associazione poliforma si è utilizzato un plugin:\verb|has_many_polymorphs|.


Ad esempio il model \verb|Classroom| ha più vincoli; sia di tipo \verb|Boolean| che di tipo \verb|Temporal|. 
Questo viene così implementato:\\
\verb|has_many_polymorphs :constraints,|\\
\verb|:from => [:boolean_constraints, :temporal_constraints]|


Ora dato un oggetto di tipo \verb|Classroom| c, per vedere tutti i suoi possibili vincoli semplicemente basta fare \verb|c.constraints|
...
\begin{itemize}
 \item \verb|belongs_to :building|: tra \verb|buildings| e \verb|classrooms| esiste un'associazione one-to-many; \verb|classrooms contiene| la chiave esterna
  \item \verb|has_many_polymorphs :constraints, :dependent => :destroy|:\\ \verb|classrooms| possiede più vincoli di diverso tipo. Se si distrugge l'oggetto, verranno distrutti anche tutti i \verb|constraints| associati
...
\item \verb|has_many_polymorphs :constraints|:\\ \verb|graduate_courses| è in associazione polimorfa con \verb|constraint|. Un oggetto di tipo \verb|GraduateCourse| può essere associato ha tre tipi di vincoli: booleani, temporali e di quantità. Se un graduate course viene distrutto, verranno cancellati dal database tutti i suoi vincoli 
\end{itemize}
...
\item \verb|has_many_polymorphs :constraints|: un oggetto di tipo \verb|Teacher| possiede più vincoli di tempo. Se viene cancellato un teacher verranno cancellati anche tutti i vincoli associati.
\end{itemize}
