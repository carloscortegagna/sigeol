...
public class ItcSolver {
    ...
    public static boolean init(String inputFile, String maxTime, String seed) {
        if (inputFile == null) {
            return false;
        }
        sProblem = "ctt";
        ...
        /*if (args.length>=3) {
            ...
            String name = sInputFile.getName();
            if (name.indexOf('.')>=0) name=name.substring(0, name.lastIndexOf('.'));
            sOutputFile = new File(sInputFile.getParentFile(), name+"."+ext);
        //}
    ...
    public static boolean start(String inputFile, String timeout, String seed) {
        boolean solved = false;
        try {
            if (init(inputFile, timeout, seed)) {
                Solver solver = create();
