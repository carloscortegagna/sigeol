...
public class CttRoom {
     ...
     */
    public CttRoom(CttModel model, String id, int size, String type) {
        iModel = model;
        iId = id;
        iSize = size;
    ...
    public class RoomConstraint extends Constraint {
        ...
        public RoomConstraint() {
            ...
            iNotAvailable = new boolean[iModel.getNrDays()][iModel.getNrSlotsPerDay()];
            for (int d=0;d<iModel.getNrDays();d++)
                for (int s=0;s<iModel.getNrSlotsPerDay();s++) {
                    iPlacement[d][s] = null;
                    iNotAvailable[d][s] = false;
                }
        ...
        public void computeConflicts(Value value, Set conflicts) {
            ...
            if (iNotAvailable[p.getDay()][p.getSlot()])
                iPlacement[p.getDay()][p.getSlot()] = p;
            if (iPlacement[p.getDay()][p.getSlot()]!=null && (!iPlacement[p.getDay()][p.getSlot()].variable().equals(value.variable()) || iNotAvailable[p.getDay()][p.getSlot()]))
        ...
        public boolean isConsistent(Value value1, Value value2) {
            ...
            if (iNotAvailable[p1.getDay()][p1.getSlot()])
                iPlacement[p1.getDay()][p1.getSlot()] = p1;
            if (iNotAvailable[p2.getDay()][p2.getSlot()])
                iPlacement[p2.getDay()][p2.getSlot()] = p2;
            if (!p1.getRoom().equals(CttRoom.this) && !iNotAvailable[p1.getDay()][p1.getSlot()]) return true;
            if (!p2.getRoom().equals(CttRoom.this) && !iNotAvailable[p2.getDay()][p2.getSlot()]) return true;
            return (p1.getDay()!=p2.getDay() || p1.getSlot()!=p2.getSlot()) && (!iNotAvailable[p1.getDay()][p1.getSlot()] && !iNotAvailable[p2.getDay()][p2.getSlot()]);
        }
    ...
    public void setAvailable(int d, int s) {
        iConstraint.iNotAvailable[d][s] = true;
    }
}
