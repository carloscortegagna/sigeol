...
public class MyPreferences_DaySlot {
	...
	class DaySlot_PreferenceInfo{
        ...
        public boolean equals(Object obj) {
            if ((obj instanceof DaySlot_PreferencesInfo) == false)
                return false;
            DaySlot_PreferencesInfo ds = (DaySlot_PreferencesInfo)obj;
	...
	private boolean existsCoursePreference(String CourseID) {
        for (Iterator it = DaySlot_Preferences.iterator(); it.hasNext();)
            if (((DaySlot_PreferencesInfo)it.next()).ID_Course.equals(CourseID))
                return true;
		return false;
	}	
	...
	private DaySlot_PreferenceInfo getPreference(String CourseID, int day, int slot) {
		for(int i=0; i<DaySlot_Preferences.size(); i++)
			if (((DaySlot_PreferencesInfo)DaySlot_Preferences.elementAt(i)).ID_Course.equals(CourseID) && 
					((DaySlot_PreferencesInfo)DaySlot_Preferences.elementAt(i)).day == day &&
					((DaySlot_PreferencesInfo)DaySlot_Preferences.elementAt(i)).slot == slot ) 
						return (DaySlot_PreferencesInfo)DaySlot_Preferences.elementAt(i));
		return null;
	}
	...
	public void addPreference(String CourseID, int Day, int Slot, int Priority){
		...
		DaySlot_PreferencesInfo thisPref = new DaySlot_PreferencesInfo(CourseID, Day, Slot, getPriorityValue(Priority));
	...
	public int getPreferencesPenalty(CttCourse course) {
		...
		if (existsCoursePreference(c_id))
			for (int i=0;i<course.getNrLectures();i++) {
	            ...
	            if (p!=null && thispref!=null)
	            	penality += thispref.weight;
	            }
        return penality;
	}
