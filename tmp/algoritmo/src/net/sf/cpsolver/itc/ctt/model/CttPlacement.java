...
public class CttPlacement extends Value implements TabuElement {
     ...
     */
    public int toInt() {
    	CttCourse c = ((CttLecture)variable()).getCourse();
    	int p = c.getModel().getPref().getPreferencesPenalty(c);
    	return getRoomCapPenalty()+getRoomPenalty()+getMinDaysPenalty()+getCompactPenalty()+(p*5);
    }
